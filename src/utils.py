...
def project_point3d_to_image_batch(c2ws, pts3d, fx,fy,cx,cy, device="cuda:0"):
    ...
    if c2ws.shape[-2:] != (4,4):
        c2ws = torch.cat([c2ws, bottom.to(device) if (c2ws.shape == 2) else bottom.view(1,4,1).repeat(c2ws.shape[0],1,1)],dim=-1).to(device)
...
def get_sample_uv_batch(H0, H1, W0, W1, n, idx, depth, color, device='cuda:0', indices=None):
    """
...
def get_samples_batch(H0, H1, W0, W1, n, H, W, fx, fy, cx, cy ,c2ws, idxs, depth, color, device, indices=None, return_uv=False):
    i, j, sample_depth, sample_color = get_sample_uv_batch(H0, H1, W0, W1, n, idxs, depth, color, device=device, indices=indices)
    i = i.view(-1,c2ws.shape[0])
    j = j.view(-1,c2ws.shape[0])
    rays_o, rays_d = get_rays_from_uv_batch(i,j,c2ws, H,W,fx,fy,cx,cy, device)
    if return_uv:
        return rays_o.contiguous().view(-1,3), rays_d.contiguous().view(-1,3), sample_depth.view(-1), sample_color.view(-1,3), torch.stack([i.view(-1),j.view(-1)],dim=1).to(device)
    else:
        return rays_o.view(-1,3), rays_d.view(-1,3), sample_depth.view(-1), sample_color.view(-1,3)
def get_sample_uv_by_indices_batch(H0, H1, W0, W1, depth, color, indices, idxs, device='cuda:0'):
    ...
    color = torch.nn.functional.grid_sample(color.permute(0, 3, 1, 2),
                          ...
                          mode="bilinear").permute(2,0,3,1).contiguous().view(-1,3)
...
def get_tensor_from_camera(RT, Tquad=False, method="quat"):
    ...
    if method == "quat":
        ...
        quad = rot2quaternion(torch.from_numpy(R).unsqueeze(0)).squeeze(0).detach().cpu().numpy()
    elif method == "ortho6d":
